...
note:
	making a program that can reduce your exact problem into a mathematical
	function for further use by programming it in.
	like taking a specific cos/sin function and finding the most exact 
	match to a reduced problem.

	> would be so cool.
	then we can plug that in into a generator for C code.

	if we know the questions we can be exact about the answer!
